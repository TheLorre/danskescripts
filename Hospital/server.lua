...
AddEventHandler('HOSPITAL:released', function(from)
	...
	TriggerClientEvent('chatMessage', -1, "SYSTEM", {200,0,0}, name.." er blevet løsladt fra hospitalet")
end)

RegisterServerEvent('HOSPITAL:reminder')

AddEventHandler('HOSPITAL:reminder', function(from, seconds)
	TriggerClientEvent('chatMessage', from, "SYSTEM", {200,0,0}, "Du har "..tostring(seconds).." sekunnder mere før du bliver løsladt fra hospitalet.")
end)
	...
	if(message:sub(1,1) == "/") then
		...
		if (cmd == "/hospital" or cmd == '/er') then
			...
			if(playerID == nil or GetPlayerName(playerID) == nil) then
				-- Invalid player ID
				TriggerClientEvent('chatMessage', from, "SYSTEM", {200,0,0} , "Ugyldigt ID")
				return
			end
			...
			local eventMessage = userName.." er blevet hospitaliseret i "..tostring(hospitalTime).." sekudner begrund af "..tostring(hospitalReason..".")
